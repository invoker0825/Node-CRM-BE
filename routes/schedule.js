...
router.get("/", (req, res) => {
    const query = "SELECT dbo.tbl_scheduler.*, dbo.tbl_scheduler_category.CategoryName as Name FROM dbo.tbl_scheduler LEFT JOIN dbo.tbl_scheduler_category ON dbo.tbl_scheduler.SchedulerCategoryID=dbo.tbl_scheduler_category.ID WHERE dbo.tbl_scheduler.IsDeleted=0";
    ...
    sql.query(config, query, (err, schedules) => {
        if (err) {
            res.status(500).send({
                message: err.message || "Some error occurred while retrieving Playlists."
            });
            ...
            sql.query(config, tagObjectQuery, (err, tagObjects) => {
                if (err) {
                    ...
                    const scheduleLocationQuery = "SELECT * FROM dbo.tbl_scheduler_location WHERE IsDeleted=0";
                    sql.query(config, scheduleLocationQuery, (err, scheduleLocations) => {
                        if (err) {
                            res.status(500).send({
                                message: err.message || "Some error occurred while retrieving Playlists."
                            });
                            ...
                            let temp = [];
                            schedules.forEach(pl => {
                                let tempTags = tagObjects.filter(t => t.ObjectType === 'schedule' && t.ObjectID === pl.ID);
                                let tempLocations = scheduleLocations.filter(t => t.SchedulerID === pl.ID);
                                let tempTagNames = [];
                                let tempLocationNames = [];
                                tempTags.forEach(t => {
                                    tempTagNames.push(allTags.filter(tag => tag.ID === t.TagID)[0].ID)
                                })
                                tempLocations.forEach(l => {
                                    tempLocationNames.push(l.LocationID)
                                })
                                temp.push({...pl, Tag: tempTagNames, Location: tempLocationNames});
                            })
